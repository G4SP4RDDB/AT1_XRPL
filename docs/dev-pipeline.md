...
Exit: two answers plus a decision: multisig borrower is viable, or fall back to a plain borrower with an off-chain gate and document why.
...
| 2.10 | `loan/impair.ts` | `LoanManage` `tfLoanImpair` then `tfLoanUnimpair` from the broker | `LossUnrealized` > 0 on the vault, PPS visibly lower, restored after unimpair |
