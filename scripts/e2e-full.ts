...
log("VaultWithdraw full (guardrail)", "tecINSUFFICIENT_FUNDS", await tx.withdraw({ depositorAddress: A.lender1.classicAddress, vaultId: ask.vaultId!, mode: "full" }));
...
log("VaultWithdraw yield-only", "tesSUCCESS", await tx.withdraw({ depositorAddress: A.lender1.classicAddress, vaultId: ask.vaultId!, mode: "yield-only" }));
pos = await read.position(A.lender1.classicAddress, ask.vaultId!);
...
log("VaultWithdraw all shares", "tesSUCCESS", await tx.withdraw({ depositorAddress: A.lender1.classicAddress, vaultId: ask.vaultId!, mode: "full" }));
pos = await read.position(A.lender1.classicAddress, ask.vaultId!);
