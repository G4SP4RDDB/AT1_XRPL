...
// The broker profile follows whatever BROKER_SEED is in .env (regenerated by `npm run fund:setup`);
// the three demo bank addresses are fixed. Pre-seeded so these accounts already read as
// institutions without live onboarding; a freshly connected address still goes through the modal.
const SEED_PROFILES: BankProfile[] = [
  ...
  { address: loadAccounts().broker.classicAddress, bankName: "AT1 Structuring Desk", shortCode: "AT1", country: "Ireland", logoEmoji: "🛡️", rating: "BBB+", createdAt: new Date(0).toISOString() },
];
