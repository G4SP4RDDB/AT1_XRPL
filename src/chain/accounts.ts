...
/** The backend never holds the broker's own private key: it only knows its public address. Every
 *  broker-signed transaction is authorized either by the broker's 2-of-2 multisig (see
 *  brokerOperator.ts + enforcer/index.ts) or, for LoanSet only, its RegularKey (brokerLoanSetKey.ts). */
export type BrokerHandle = { classicAddress: string };

/** Wallets for platform roles. Only BROKER_ADDRESS is required in .env (a public address, not a
 *  seed). Other roles are resolved dynamically from the database. */
export function loadAccounts(): Record<Exclude<Role, "broker">, Wallet> & { broker: BrokerHandle } {
  const env = readEnv();
  const out = {} as Record<Exclude<Role, "broker">, Wallet> & { broker: BrokerHandle };

  if (!env.BROKER_ADDRESS) {
    throw new Error("missing BROKER_ADDRESS in .env: platform broker's public address is required (never a seed — see scripts/setup-broker-multisig.ts)");
  }
  out.broker = { classicAddress: env.BROKER_ADDRESS };
